...
const fetchCategoryInfo = async () => {
  try {
    const result = await categoryApi.getCategoryById(categoryId.value);
  ...
  <div class="category-list-container">
    <div class="content-wrapper">
      <div class="category-header">
        ...
        <div class="category-description" v-if="category.description">{{ category.description }}</div>
        <div class="category-meta" v-if="category.id">
          <span><TagOutlined /> 分类ID: {{ category.id }}</span>
          <span><EyeOutlined /> 文章数量: {{ pagination.total }}</span>
          <span><CalendarOutlined /> 创建时间: {{ category.createTime }}</span>
        </div>
      </div>
